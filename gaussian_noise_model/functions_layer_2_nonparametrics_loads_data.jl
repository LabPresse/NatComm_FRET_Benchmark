...
function get_log_observation_prob(bin::Int64,
    ...
    FRET_eff::Float64 = rates[(state_label-1)*max_n_system_states+state_label]

    # Shot noise
    log_observation_prob::Float64 = logpdf(Normal(
            mean_donor[state_label], sqrt(variance_donor[state_label])),
        donor_channel_data[bin]) +
                                    logpdf(Normal(
            mean_acceptor[state_label], sqrt(variance_acceptor[state_label])),
