...
#	state_trajectory = get_gt_trajectory()
 	...
 	for bin in 1:nbins
 		...
 		FRET_eff = rates[(loads_active[next_state] - 1)*
 				 max_n_system_states + loads_active[next_state]]
 	end

	return loads, rates, state_trajectory,
			mean_donor, mean_acceptor
end
...
function sample_loads_state_trajectory(draw::Int64,
	...
	state_trajectory_sampled, accept_trajectory =
...
#	return loads, old_state_trajectory
 	return loads, state_trajectory_sampled
end
...
function sample_transition_rates(draw::Int64,
	...
	covariance_rates::Float64 = 1.0
	conc_parameter_FRET::Float64 = 1.0e2
		...
		for j in loads_active
			...
			if i != j  # For Confo Rates
    			...
    			proposed_rates = copy(rates)
 				proposed_rates[ij] =
						rand(Normal(log(rates[ij]), covariance_rates),1)[1]
	...
	for i in 1:max_n_system_states
		...
		elseif loads[i] == 0
			...
			if proposed_mean_donor[i] > 0.0
 				...
 				log_hastings =
 					new_log_conditional_posterior - old_log_conditional_posterior
 				if log_hastings >= log(rand()) && proposed_mean_donor[i] > 0.0
 					mean_donor = copy(proposed_mean_donor)
 				end
			end
			...
			if proposed_mean_acceptor[i] > 0.0
 				...
 				log_hastings =
					new_log_conditional_posterior - old_log_conditional_posterior
 				if log_hastings >= log(rand()) && proposed_mean_acceptor[i] > 0.0
 					mean_acceptor = copy(proposed_mean_acceptor)
 				end
			end
